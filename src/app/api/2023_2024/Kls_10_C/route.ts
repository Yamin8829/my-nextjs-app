...
export async function GET(request: NextRequest) {
  ...
  const directoryPath = path.join(
    process.cwd(),
    "public/images/fotbarSMA/2023_2024/Kls_10_C"
  );
  ...
  const data = dataFile.map((image) => {
    return {
      key: image.filename,
      url: `/images/fotbarSMA/2023_2024/Kls_10_C/${image.filename}`,
      alt: image.filename,
      album_id: "2023_2024/Kls_10_C",
  ...
  return NextResponse.json({
    status: 200,
    message: "Hello World Kelas 10 C tai",
    data: data,
  });
}
