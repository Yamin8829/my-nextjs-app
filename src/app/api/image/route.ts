...
export async function GET(request: NextRequest) {
  ...
  const data = dataFile.map((image) => {
    return {
      key: image.filename,
      url: `/images/fotbarSMA/2023_2024/Kls_10_A/${image.filename}`,
      alt: image.filename,
      album_id: "2023_2024/Kls_10_A",
  ...
  // const filePath = path.join(process.cwd(), "public", "data","2023_2024", "10_A.json");
  ...
  return NextResponse.json({
    status: 200,
    message: "Hello World Kelas 10 A tai",
    data: data,
  });
}
